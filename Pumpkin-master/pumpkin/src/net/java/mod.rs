    ...
    pub async fn handle_play_packet(
        ...
        match packet.id {
            ...
            id if id == SCustomPayload::to_id(version) => {
                ...
                let data = Bytes::from(payload.data);
                server
                    .mythicraft
                    .handle_custom_payload(server, player.clone(), &channel, &data)
                    .await;
                let event = PlayerCustomPayloadEvent::new(player.clone(), channel, data);
