...
use crate::mythicraft::MythicraftCore;
...
use std::{future::Future, path::Path, sync::atomic::Ordering, time::Duration};
...
pub struct Server {
    ...
    /// Built-in Mythicraft RPG, economy, permission and client-payload core.
    pub mythicraft: MythicraftCore,
...
impl Server {
        ...
        send_cancellable! {{
            ...
            'after: {
                ...
                if world
                    .add_player(&player)
                    .is_ok() {
                    self.mythicraft.on_player_join(&player).await;
    ...
    pub async fn remove_player(&self, player: &Player) {
        self.mythicraft.on_player_leave(player).await;
        player
            .increment_stat(
