...
impl Ticker {
    ...
    pub async fn run(server: &Arc<Server>) {
        ...
        'ticker: loop {
            ...
            // RPG state is advanced only after Pumpkin has completed its authoritative
            // player/world tick, so Mythicraft cannot observe a partially updated frame.
            server
                .mythicraft
                .tick(server, tick_number, tick_duration_nanos)
                .await;
